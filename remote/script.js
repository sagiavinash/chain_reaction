...
$(document).ready(function() {
	...
	function poll_opponent_move() {
		...
		$.ajax({
			...
			if (response && response.time !== poll_opponent_move.old) {
				...
				$(".action-log").append("<p>Player " + opid + "'s move: " + response.move);
			} else {
	...
	function record_move(boxIndex){
		$(".action-log").append("<p>Player " + pid + "'s move: " + boxIndex);
		$.ajax({
		...
		return function() {
			if (isGridActive) {
				...
				if (validTurn && !matchWon) {
					...
					$(".current-player .player-name").html((currentplayer + 1 === pid) ? "Your Turn" : ("Player " + (currentplayer + 1) + "'s Turn"));
					validTurn = false;
					firstTurn = false;
				}
			}
